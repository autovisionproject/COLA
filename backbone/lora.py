...
class timm_cola(nn.Module):
    def __init__(self, vit_model: timm_ViT, r: int, num_classes: int = 0, increment=10, filepath = './', lora_layer=None, eval=False, index=True, cur_task_index=None,device="cuda"):
        super(timm_cola, self).__init__()
        ...
        for t_layer_i, blk in enumerate(vit_model.blocks):
            ...
            self.w_As.append(w_a_linear_v)
            self.w_Bs.append(w_b_linear_v)
            blk.attn.qkv = timm_cola_train(w_qkv_linear, w_a_linear_q, w_b_linear_q, w_a_linear_v, w_b_linear_v, self.task_id, t_layer_i, self.rank, self.scale, self.device)
    ...
    def reset_sigma(self):
        for layer_i, blk in enumerate(self.lora_vit.blocks):
            if isinstance(blk.attn.qkv, timm_cola_train):
                for oja in [blk.attn.qkv.oja_q, blk.attn.qkv.oja_v]:
                    oja.Sigma_task.copy_(oja.Sigma_global.clone()+oja.Sigma_task.clone())
                    oja._task_batches = 0
    ...
    def set_oja_eval_mode(self,model, eval=True):
        for blk in model.backbone.lora_vit.blocks:
            if hasattr(blk.attn.qkv, "oja_q"):
                blk.attn.qkv.oja_q.eval_mode=True
            if hasattr(blk.attn.qkv, "oja_v"):
                blk.attn.qkv.oja_v.eval_mode=True
...
class OjaContinualAdapter(nn.Module):
    def __init__(self, dim, lr=1e-4, decay=0.01, consolidation_rate=0.3, device='cuda', 
        ...
        self.lr = lr
        self.decay = decay
        self.lam = consolidation_rate
        self.device = device
        self.consolidate_every = consolidate_every
        self.proj_scale = proj_scale
        self.eval_mode = False 
        eps = 5e-4
        self.register_buffer("Sigma_global", eps * torch.eye(dim, device=self.device))
        self.register_buffer("Sigma_task", eps * torch.eye(dim, device=self.device))
        self._task_batches = 0

    def forward(self, X):
        X = X.to(self.Sigma_task.device)
        with torch.no_grad():
            Sigma_g_sym = (self.Sigma_global + self.Sigma_global.t()) / 2.0 
            norm = Sigma_g_sym.norm()
            P = torch.eye(self.dim, device=X.device) - self.proj_scale * (Sigma_g_sym / norm) 
            X_proj = torch.einsum('bsi,ij->bsj', X, P) 
            if self.eval_mode:
                return X_proj

            X_flat = X_proj.reshape(-1, self.dim)
            X_centered = X_flat - X_flat.mean(dim=0, keepdim=True)
            batch_outer = (X_centered.T @ X_centered) / X_centered.shape[0]
            update_term = batch_outer - self.decay * self.Sigma_task
            self.Sigma_task += self.lr * update_term
            self.Sigma_task = (self.Sigma_task + self.Sigma_task.t()) / 2.0

            self._task_batches += 1
            if self._task_batches % self.consolidate_every == 0:
                self.Sigma_global += self.lam * (self.Sigma_task - self.Sigma_global)
                self.Sigma_global = (self.
                Sigma_global + self.Sigma_global.t()) / 2.0
        return X_proj


class timm_cola_train(nn.Module):
    def __init__(self, qkv, linear_a_q, linear_b_q, linear_a_v, linear_b_v, task_id, t_layer_i, rank, scale, device):
        super().__init__()
        self.device=device
        ...
        self.t_layer_i = t_layer_i
        self.rank = rank
        self.oja_q = OjaContinualAdapter(dim=self.rank,device=self.device)
        self.oja_v = OjaContinualAdapter(dim=self.rank,device=self.device)

    def forward(self, x):
        z_q = self.linear_a_q(x)
        z_v = self.linear_a_v(x)
        sigma_q = self.oja_q(z_q)
        sigma_v = self.oja_v(z_v)
        sigma_q = sigma_q + self.scale * z_q
        sigma_v = sigma_v + self.scale *  z_v
        lora_q =  self.linear_b_q(sigma_q)
        lora_v =  self.linear_b_v(sigma_v)

        lora_q =  self.linear_b_q(z_q)
        lora_v =  self.linear_b_v(z_v)
        qkv_out = self.qkv(x)
        qkv_out[:, :, :self.dim] +=  lora_q
        qkv_out[:, :, -self.dim:] += lora_v
        return qkv_out
