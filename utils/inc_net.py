...
import timm
from backbone.lora import LoRA_ViT_timm,timm_cola

def get_backbone(args, pretrained=False):
    name = args["backbone_type"].lower()
    ## Lora version
    if  args['model_name']=="cola":
        ...
        model = timm_cola(vit_model=model.eval(), r=args['rank'],  num_classes=args['num_classes'], increment=args['increment'], filepath=args['filepath'])
        #model = nn.DataParallel(model)
        model.out_dim = 768

        return model

    elif  args['model_name']=="sdlora":
        model = timm.create_model(args['backbone_type'], pretrained=True, num_classes=0)
